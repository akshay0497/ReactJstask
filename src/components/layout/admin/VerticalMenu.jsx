...
const MenuItemWithChildren = ({
  ...
  return (
    <li className={className}>
      <button
        className="hs-accordion-toggle flex w-full items-center gap-x-3.5 rounded-md px-4 py-3 text-sm text-default-700 hover:bg-default-100"
        aria-expanded={open}
...
const VerticalMenu = ({ menuItems }) => {
  ...
  const activeMenu = useCallback(() => {
    const matchingMenuItem = getMenuItemFromURL(menuItems, window.location.pathname.replaceAll("", ""));
    if (matchingMenuItem) {
